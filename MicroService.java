...
import bgu.spl.mics.application.passiveObjects.ServicesCounter;
...
public abstract class MicroService implements Runnable {
    ...
    private MessageBusImpl mbInst = MessageBusImpl.getInstance();
    private ConcurrentHashMap<Class<? extends Message>, Callback> mapCallback = new ConcurrentHashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        mapCallback.put(type, callback);
        mbInst.subscribeEvent(type, this);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        mapCallback.put(type, callback);
        mbInst.subscribeBroadcast(type, this);
    }
    ...
    public final void run() {
        mbInst.register(this);
        initialize();
        ServicesCounter.getInstance().Increment();
        while (!terminated) {
            try{
                Message m = mbInst.awaitMessage(this);
                mapCallback.get(m.getClass()).call(m);
            }
